...
class ViT(nn.Module):
    ...
    def update_spectral_terms(self):
        with torch.no_grad():
            ...
            for i, module in enumerate(self.transformer.layers):
                ...
                self.spectral_precompute *= (self.q_sn[i] * self.k_sn[i] * self.v_sn[i] * self.w_sn[i] * self.wp_sn[i] * self.rho_sfmx) ** (2 ** (self.depth-i) - 1)
    ...
    def spectral_complexity(self, x):
        with torch.no_grad():
            ...
            spectral_term = self.spectral_precompute * xfn * (xsn ** (2 ** (self.depth+1) - 1))
